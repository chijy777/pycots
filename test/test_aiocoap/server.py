...
class BlockResource(resource.Resource):
    ...
    async def render_put(self, request):
        ...
        return aiocoap.Message(
            code=aiocoap.Code.CHANGED, payload=self.content
        )
...
def main():
    ...
    root.add_resource(
        ('other', 'block'), BlockResource()
    )
    root.add_resource(
        ('other', 'separate'), SeparateLargeResource()
    )
    print(root)

    asyncio.Task(aiocoap.Context.create_server_context(
        site=root, bind=(cfg.SERVER_IP, aiocoap.COAP_PORT))
    )
