...
from pycots.common.auth import DEFAULT_KEY_FILENAME

logging.basicConfig(
    level=logging.DEBUG,
    format='%(asctime)s - %(name)14s - ' '%(levelname)5s - %(message)s'
)
logger = logging.getLogger("pycots.helpers")
...
def start_application(app, port=None, close_client=False):
    ...
    signal.signal(
        signal.SIGTERM, partial(signal_handler, _server, app.close_client)
    )
    signal.signal(
        signal.SIGINT, partial(signal_handler, _server, app.close_client)
    )
