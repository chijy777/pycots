...
logger = logging.getLogger("mqtt_client_test")
...
NODE_RESOURCES = {
    'name': {'delay': 0, 'value': lambda x=None: "MQTT test node"},
    'os': {'delay': 0, 'value': lambda x=None: "riot"},
    'ip': {'delay': 0, 'value': (lambda x=None: socket.gethostbyname(socket.gethostname()))},
    'board': {'delay': 0, 'value': lambda x=None: "HP"},
    'led': {'delay': 0, 'value': lambda x=None: LED_VALUE},
    'temperature': {'delay': 5, 'value': (lambda x=None: '{}°C'.format(random.randrange(20, 30, 1)))},
    'pressure': {'delay': 10, 'value': (lambda x=None: '{}hPa'.format(random.randrange(990, 1015, 1)))}
}
...
def send_check(mqtt_client):
    while True:
        ...
        asyncio.get_event_loop().create_task(
            publish(mqtt_client, 'node/check', check_data)
        )
...
def start_client():
    ...
    yield from mqtt_client.subscribe(
        [('gateway/{}/discover'.format(NODE_ID), QOS_1)]
    )
    yield from mqtt_client.subscribe(
        [('gateway/{}/led/set'.format(NODE_ID), QOS_1)]
    )
    asyncio.get_event_loop().create_task(
        send_check(mqtt_client)
    )
    asyncio.get_event_loop().create_task(
        send_values(mqtt_client)
    )
    ...
    while True:
        ...
        logger.debug(
            "Received message from gateway: {} => {}".format(topic_name, data)
        )
...
def publish_continuous(mqtt_client, topic, value, delay=0):
    while True:
        ...
        yield from mqtt_client.publish(
            topic, data.encode('utf-8'), qos=QOS_1
        )
        logger.debug(
            "Published '{}' to topic '{}'".format(data, topic)
        )
        if delay == 0:
            break
